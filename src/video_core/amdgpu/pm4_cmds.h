...
struct PM4CmdSetPredication {
    ...
    /// DW1 holds the full lower 32 bits of the address (Mesa/RADV encodes as va & 0xFFFFFFFF).
    /// The address must be u64-aligned so bits [2:0] are always zero in valid packets,
    /// but we use raw_lo directly rather than masking them away via a BitField.
    VAddr Address() const {
        return u64(raw_lo) | (u64(bool_addr_hi.Value()) << 32);
    }
};

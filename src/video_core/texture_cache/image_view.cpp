...
ImageViewInfo::ImageViewInfo(const AmdGpu::Image& image, const Shader::ImageResource& desc) noexcept
    ...
    range.base.layer = image.base_array < image.NumLayers() ? image.base_array : 0u;
...
ImageViewInfo::ImageViewInfo(const AmdGpu::ColorBuffer& col_buffer) noexcept {
    ...
    const u32 num_slices = col_buffer.NumSlices();
    range.base.layer = base_slice < num_slices ? base_slice : 0u;
    range.extent.layers = num_slices - range.base.layer;
...
ImageViewInfo::ImageViewInfo(const AmdGpu::DepthBuffer& depth_buffer, AmdGpu::DepthView view,
    ...
    const u32 num_slices = view.NumSlices();
    range.base.layer = base_slice < num_slices ? base_slice : 0u;
    range.extent.layers = num_slices - range.base.layer;
    type = range.extent.layers > 1 ? AmdGpu::ImageType::Color2DArray : AmdGpu::ImageType::Color2D;
}
